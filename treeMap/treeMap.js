...
Tree.prototype.map = function (callback) {
  ...
  var makeNode = function(oldNode, callback){
    var newNode = new Tree;
    newNode.value = callback(oldNode.value);
  }

  var makeTree = function(oldTree, callback){
    var newTree = makeNode(oldTree, callback);
    if (oldTree.children.length > 0){
      for (var i = 0 ; i < oldTree.children.length ; i++){
        makeTree(oldTree.children[i]);
      }
    }
    return newTree;
  }

  return makeTree(this, callback);
