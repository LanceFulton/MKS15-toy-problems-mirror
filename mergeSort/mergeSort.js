 ...
 *
 * Complexity:
 *   What is the complexity of your algorithm in time and space - space: linear in iterative, quadratic in recursive solution
     time - linear in iterative, log n in recursive?
 ...
 *   complexity is (spoiler alert!) much lower. Why is that?
     We save time by looping only over the first digits of each sorted-sub array-to-be-resorted, as opposed to every digit in the pair to be
...
// need to acct for edge cases, debug

var mergeSort = function(array) {
  var i = array.length;
  var arrayLeft = array.slice(0, i/2);
  var arrayRight = array.slice(i/2, array.length);
  if (arrayToSort.length > 1){
    mergeSort(array)
  }
  if (arrayLeft < arrayRight) {
    return arrayLeft.push(arrayRight);
  } else {
    return arrayRight.push(arrayLeft);
  }
};
