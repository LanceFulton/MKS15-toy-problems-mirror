 ...
 *     There is an algorithm that uses O(N) time and O(1) space (i.e., an in-place shuffle).
       I believe my algorithm is o(n) time, o(1) space complexity.
 *
...
var shuffleDeck = function(deck) {
  ...
  for (var i = 0; i < deck.length; i++){
    var randA = Math.floor(Math.random(1)*52);
    var randB = Math.floor(Math.random(1)*52);
    temp = shuffledDeck[randA];
    shuffledDeck[randA] = shuffledDeck[randB];
    shuffledDeck[randB] = temp;
  }
  return shuffledDeck;
};
